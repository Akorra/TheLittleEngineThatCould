...
{
public:
    enum class PolyMode  { FrontAndBack, Front, Back };
    enum class RastMode  { Fill, Point, Line };
    ...
    // Shader
    void SetProgram(ShaderHandle program) { program_ = program; }
    ...
    const float GetFloat(const std::string& name, float defaultValue = 0.0f) const;
    const int GetInt(const std::string& name, int defaultValue = 0) const;
    const Vec2 GetVec2(const std::string& name, const Vec2& defaultValue = Vec2(0)) const;
    const Vec3 GetVec3(const std::string& name, const Vec3& defaultValue = Vec3(0)) const;
    const Vec4 GetVec4(const std::string& name, const Vec4& defaultValue = Vec4(0)) const;
    const Mat3 GetMat3(const std::string& name, const Mat3& defaultValue = Mat3(1)) const;
    ...
    bool IsTransparent() const { return blendMode_ != BlendMode::Opaque; }
    
    void SetRenderQueue(int queue) { queue_ = queue; }
    int GetRenderQueue() const { return queue_; }
...
private:
    ...
    // Render state
    PolyMode  polygonMode_ = PolyMode::FrontAndBack;
    RastMode  rastMode_    = RastMode::Fill;
    CullMode  cullMode_    = CullMode::Back;
    BlendMode blendMode_   = BlendMode::Opaque;
    DepthFunc depthFunc_   = DepthFunc::LessEqual;

    bool depthTest_  = true;
    bool depthWrite_ = true;
    int  queue_      = 2000; //< default opaque
