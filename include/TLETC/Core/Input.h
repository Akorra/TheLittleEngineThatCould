 ...
 * No framework access, pure state.
 */
class Input 
{
public:
    ...
    vec2 GetScrollDelta() const { return scrollDelta_; }
...
private:
    ...
    ButtonState keys_[MAX_KEYS];
    ButtonState mouseButtons_[MAX_MOUSE_BUTTONS];

    vec2 mousePosition_{0.0f};
    vec2 lastMousePosition_{0.0f};
    vec2 mouseDelta_{0.0f};
    vec2 scrollDelta_{0.0f};
};
